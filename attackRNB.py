...
if __name__=="__main__":
    ...
    ### Change config of script to compatible with TF.1.
    ...
    for i in range(0,len(x_len)):
        text.append(dataset.build_text(orig_list[i][:x_len[i]]))
    orig_list = text
    
    x_len = np.sum(np.sign(adv_list),axis=-1)
    text = []
    for i in range(0,len(x_len)):
        text.append(dataset.build_text(adv_list[i][:x_len[i]]))
    adv_list = text
    
    ### Write out results.
    df = pd.DataFrame(list(zip(orig_list,adv_list,dist_list)),
               columns =['Original_text', 'Adverserial_text','Percent_changed'])
    df.to_csv('RNB_result.csv', index=False)
