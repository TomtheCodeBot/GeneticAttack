...
class RobustNaiveBayesClassifierPercentage(BaseEstimator):
    ...
    def _solve_subproblem(self, f, kappa):
        ...
        for k in range(1, n + 1):
            if k == n:
                h_curr = h_curr + f_new[0, k] * np.log(f_new[0, k]) - f_new[0, k]
                F_curr = h_curr + kappa * np.log(f_new[0, k])
                v_curr = f_new[0, k]
            else:
                rho_curr = rho_curr - f_new[0, k]
                h_curr = h_curr + f_new[0, k] * np.log(f_new[0, k]) - f_new[0, k]
                v_curr = min(f_new[0, k], max(f_new[0, k + 1], rho_curr/(n - k)))
    ...
    def fit(self, X, y):
        ...
        if (len(self.theta_pos.shape) < 2):
            self.theta_pos = np.reshape(self.theta_pos, (1, self.theta_pos.shape[0]))
        
        if (len(self.theta_neg.shape) < 2):
            self.theta_neg = np.reshape(self.theta_neg, (1, self.theta_neg.shape[0]))
...
#         pos_prob = np.log(self.pos_prior_probability) + np.sum(xlogy(X, self.theta_pos), axis=-1)
#         neg_prob = np.log(self.neg_prior_probability) + np.sum(xlogy(X, self.theta_neg), axis=-1)
        
#         print(X.shape,self.theta_pos.shape)
        pos_prob = np.log(self.pos_prior_probability) + np.sum(X@np.log(self.theta_pos).T, axis=-1)
        neg_prob = np.log(self.neg_prior_probability) + np.sum(X@np.log(self.theta_neg).T, axis=-1)
...
#         print(X.shape,self.theta_pos.shape)
        ...
        exp_pos= np.expand_dims(np.exp(pos_prob),axis=-1)
        exp_neg=np.expand_dims(np.exp(neg_prob),axis=-1)
        
        output = np.concatenate((exp_neg,exp_pos),axis=1)
        try:
            min_value = np.nanmin(output[np.nonzero(output)])
        except ValueError:  #raised if `y` is empty.
            min_value=0.5
            pass
        
        
        exp_pos = np.nan_to_num(exp_pos,nan=min_value)
        exp_pos[exp_pos==0]=min_value
        exp_neg = np.nan_to_num(exp_neg,nan=min_value)
        exp_neg[exp_neg==0]=min_value
        
        exp_sum =np.squeeze( exp_pos+exp_neg)
        ...
        predictions = output/exp_sum[:,np.newaxis]
